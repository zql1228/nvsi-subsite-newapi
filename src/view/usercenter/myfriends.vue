    ...
    <div class="tabbar-down">
      <table class="user-table">
        ...
        <tr v-for="(el, index) in friendsList" :key="index">
          <td class="left">
            <em
              @click="toUserDetail(el.albp1802, el.albp1803)"
              class="pointer name_color"
              >{{
                el.albp1808 == userInfo.albp0064 ? el.albp1809 : el.albp1808
              }}</em
            >
          </td>
          <td class="left">
            {{ el.albp1802 == userInfo.albp0052 ? el.albp1803 : el.albp1802 }}
          </td>
          <td class="left">
            <em
              class="pointer"
              @click="sendMessage(el.albp1802, el.albp1803)"
    ...
    <div class="tabbar-down" style="display:none;">
      <table class="user-table">
        ...
        <tr v-for="(el, index) in myOutList" :key="index">
          <td class="left">
            <em
              @click="toUserDetail(el.albp1802, null)"
              class="pointer name_color"
              >{{ el.albp1808 }}</em
            >
          </td>
          ...
          <td class="left">
            <em class="pointer" @click="updateState(el.id, 4)">{{
              (el.albp1806 == "1" && "取消申请") ||
                (el.albp1806 == "3" && "删除")
            }}</em>
          </td>
    ...
    <div class="tabbar-down" style="display:none;">
      <table class="user-table">
        ...
        <tr v-for="(el, index) in myInList" :key="index">
          <td class="left">
            <em
              @click="toUserDetail(null, el.albp1803)"
              class="pointer name_color"
              >{{ el.albp1809 }}</em
            >
          </td>
...
export default {
  ...
  methods: {
    ...
    deleteFriend: function(id) {
      //删除好友
      if (this.userInfo) {
        this.$swal("确认删除该好友？", { buttons: ["取消", "确定"] }).then(
          value => {
            if (value) {
    ...
    updateState: function(id, state) {
      if (this.userInfo) {
        this.utilscommit.request(
          "nvsi_updateFriendState",
          { id: id, state: state },
          function(result) {
            if (result) {
              if (result.data) {
                Vues.$swal("操作成功", { buttons: "确定" }).then(function(
                  value
                ) {
                  Vues.getList();
                });
            ...
            if (result.data) {
              window.open(
                Vue.$router.resolve({
                  name: "volinfo",
                  params: {
                    define: Vues.routeDefine,
                    data1: result.data.albp0029,
                    data2: result.data.albp0020
                  }
                }).href
              );
            } else {
    ...
    friendCheck: function() {
      ...
      if (!this.friendName) {
        ...
        this.utilscommit.request(
          "nvsi_getUserByUserName",
          { albp0052: this.friendName },
          function(result) {
            if (result && result.data) {
              tip = "";
