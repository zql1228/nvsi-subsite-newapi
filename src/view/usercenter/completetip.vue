      ...
      >
        ...
        <div style="padding: 20px 50px 0 50px">
          <div class="form" :class="{ hiddens: showtype > 1 }">
            <p class="form-label" style="text-align: center; font-size: 18px">
              <em>请先完善基本信息</em>
            </p>
            <p class="form-label" style="text-align: center; font-size: 14px">
              完善基本信息，将拥有参加团体和项目等功能的权限
            </p>
          </div>
          <div class="form" :class="{ hiddens: showtype < 4 }">
            <p
              class="form-label"
              style="text-align: center; font-size: 18px; white-space: normal"
            >
              <em
                >为了便于您参与志愿服务，请完善您的“服务区域”和“居住区域”到“区（县）”</em
              >
            </p>
          </div>
          <div class="form" :class="{ hiddens: showtype == 1 }">
            <p class="form-label" style="text-align: center; font-size: 18px">
              <em v-text="showtitle"></em>
            </p>
            <p
              class="form-label"
          ...
          <div class="toComplete" :class="{ hiddens: showtype != 4 }">
            <a href="javascript:void(0);" class="button" @click="toComplete"
              >确认</a
            >
          </div>
        </div>
...
export default {
  name: "completetip",
  props: ["showtype", "basicInfo", "albp0010"], //showtype-->弹出层展示类型 0=不展示  1=展示，未完善信息    2=展示，异站点信息
  data() {
    return {
  ...
  computed: {
    complete: function () {
      if (this.showtype == 2) {
        ...
        this.showtitle =
          "是否把志愿服务记录从“" + areaname + "”转移到“" + nowsitename + "”？";
        // this.showname="在“"+nowsitename+"”中请重新设置志愿者所属服务区域、服务类别、服务领域等信息";
        this.showname =
          "选择“是”请在“" + nowsitename + "志愿服务”子站中设置服务区域";
      } else if (this.showtype == 3) {
        this.showtitle = "请进入实名认证页面进行认证";
        this.showname = this.basicInfo.albp0063
          ? "认证原因：实名认证失败或者人工审核不通过"
          : "";
      }
      if (this.showtype) {
        //判断页面是否展示
