<template>
  <div id="areaDialog">
    <div id="preButton" style="display: none; margin-top: 20px">
      <a href="javascript:void(0);" class="button" @click="preSelect">返回上一级</a>
    </div>
    <div class="location">
      <span class="v-m" v-text="preName"></span>
    </div>
    <div class="category__content" id="first">
      <a href="javascript:void(0);" class="category__item" v-for="(el, index) in firstList" @click="firstChoose" :id="el.areaid" :title="el.areaname" :key="index">
        <span v-text="el.areaname"></span>
      </a>
    </div>
    <div class="category__content" id="second" style="display: none">
      <a href="javascript:void(0);" class="category__item" v-for="(el, index) in secondList" @click="secondChoose" v-bind:id="el.areaid" v-bind:title="el.areaname" :key="index">
        <span v-text="el.areaname"></span>
      </a>
    </div>
    <div class="category__content" id="third" style="display: none">
      <a href="javascript:void(0);" class="category__item" v-for="(el, index) in thirdList" @click="thirdChoose" v-bind:id="el.areaid" v-bind:title="el.areaname" :key="index">
        <span v-text="el.areaname"></span>
      </a>
    </div>
    <div class="category__content" id="fourth" style="display: none">
      <a href="javascript:void(0);" class="category__item" v-for="(el, index) in fourthList" @click="fourthChoose" v-bind:id="el.areaid" v-bind:title="el.areaname" :key="index">
        <span v-text="el.areaname"></span>
      </a>
    </div>
    <div class="category__content" id="fifth" style="display: none">
      <a href="javascript:void(0);" class="category__item" v-for="(el, index) in fifthList" @click="fifthChoose" v-bind:id="el.areaid" v-bind:title="el.areaname" :key="index">
        <span v-text="el.areaname"></span>
      </a>
    </div>
    <div class="form t-c">
      <a href="javascript:void(0);" class="button" @click="onSelect">确认选择</a>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      type: 0,
      areaid: "",
      isRestriction: 5, //限制级别，1-省级 2-市级 3-区县级 4街道级 5村居级
      areaCityList: "50,31,12,11", //直辖市区域编号前两位 （重庆市、上海市、天津市、北京市）
    };
  },
  created() {},
  methods: {
    listArea(areaid, type, isRestriction) {
      //查询对应的下级区域
      this.type = type;
      this.areaid = areaid;
    ...
    nextAreaBack(result) {
      if (result && result.data) {
        if (this.type == 0) {
          ...
          if (this.areaid) {
            if (this.areaid == "000000000000000000" && this.areaType != 1) {
              this.preName = "全国";
    ...
    firstChoose(a) {
      ...
      console.log(this.isRestriction)
      //  this.isRestriction = 1
      if (this.isRestriction == 1) {
        $(".category__item").removeClass("active");
        $("#" + this.first.areaid).addClass("active");
      } else {
    ...
    secondChoose(a) {
      ...
      if (this.areaCityList.indexOf(this.preAreaId.substring(0, 2)) > -1)
    ...
    onSelect() {
      //确认选择
      this.$parent.confirmArea(this.preAreaId); //调用父元素总的函数
      ...
      if (this.$route.name == "updatedata") {
        this.$parent.formTip("liveAddress", "");
      }
    },
  },
};
